"""
Database module for Smart Visitor Management System — Fedel Solutions Limited.
Handles all SQLite operations, schema creation, and data access.

PASSWORD NOTE:
  Passwords are bcrypt-hashed and stored permanently in SQLite.
  There is NO expiry, NO time-based invalidation, NO session timeout on the
  password itself.  must_change_pw=0 for all seeded accounts means no
  forced-change prompt after first login.
"""
...
#   Super Admin    — full access including approval actions
#   Admin          — full access including approval actions
#   CISO           — full access including approval actions
#   Security       — read-only: can view dashboard & approval status,
#                    CANNOT approve / reject visitors
APPROVAL_ROLES = {"Super Admin", "Admin", "CISO"}
VIEW_ONLY_ROLES = {"Security"}
...
def init_db():
    """Create all tables and seed permanent accounts on first run."""
    for d in ["data/visitors", "data/photos", "data/logs",
              "data/approvals", "data/exports", "data/backups"]:
    ...
    # ── Admins ────────────────────────────────────────────────────────────────
    # must_change_pw = 0  →  password is permanent, never forces a re-set
    # is_active      = 1  →  account is enabled forever
    c.execute("""
    ...
    # ── Permanent seeded accounts ─────────────────────────────────────────────
    # These four accounts always exist with fixed credentials.
    # must_change_pw=0  →  no forced-change prompt ever.
    # Passwords are bcrypt-hashed in the DB — NOT stored as plain text.
    seed_admins = [
        # (username,      password,       full_name,                  role)
        ("superadmin",  "SuperAdmin@123",  "Super Administrator",      "Super Admin"),
        ("admin",       "Admin@123",       "System Administrator",     "Admin"),
        ("ciso",        "CISO@Fedel2025",  "Chief Information Security Officer", "CISO"),
        ("security",    "Security@123",    "Security Officer",         "Security"),
    ]
    for uname, pw, full, role in seed_admins:
        existing = c.execute(
            "SELECT id FROM admins WHERE username=?", (uname,)).fetchone()
        if not existing:
            c.execute("""
                INSERT INTO admins(username, password_hash, full_name, role, must_change_pw, is_active)
                VALUES (?, ?, ?, ?, 0, 1)
            """, (uname, _hash(pw), full, role))
        else:
            # Ensure existing accounts are never locked out —
            # reset must_change_pw and is_active to safe values.
            c.execute("""
                UPDATE admins SET must_change_pw=0, is_active=1
...
def get_visitor_by_number(visitor_number: str):
    conn = get_connection()
    row = conn.execute(
        "SELECT * FROM visitors WHERE visitor_number=?", (visitor_number,)).fetchone()
    conn.close()
    return dict(row) if row else None
...
def record_exit(visitor_id: int):
    now = lagos_now()
    conn = get_connection()
    visitor = dict(conn.execute("SELECT * FROM visitors WHERE id=?", (visitor_id,)).fetchone())
    duration = ""
    ...
    """, (now.strftime("%Y-%m-%d"), now.strftime("%H:%M:%S"),
          now.isoformat(), duration, visitor_id))
    conn.commit()
    conn.close()
    return duration
...
def get_approvals(visitor_id=None):
    ...
    if visitor_id:
        rows = conn.execute(
            "SELECT * FROM approvals WHERE visitor_id=? ORDER BY created_at DESC",
            (visitor_id,)).fetchall()
    else:
        rows = conn.execute(
...
def get_dashboard_stats():
    ...
    stats["inside"]       = conn.execute(
        "SELECT COUNT(*) FROM visitors WHERE status='Approved' AND entry_date=?",
        (today,)).fetchone()[0]
    ...
    stats["checked_out"]  = conn.execute(
        "SELECT COUNT(*) FROM visitors WHERE status='Checked Out' AND entry_date=?",
        (today,)).fetchone()[0]
...
def verify_admin(username: str, password: str):
    ...
    Passwords are NEVER expired — bcrypt check is the only gate.
    """
    conn = get_connection()
    row = conn.execute(
...
def update_admin_password(username: str, new_password: str):
    """Hash and store a new password. Sets must_change_pw=0 permanently."""
...
def get_admins():
    conn = get_connection()
    rows = conn.execute(
        "SELECT id, username, full_name, role, email, is_active, last_login, must_change_pw "
        "FROM admins ORDER BY id").fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def add_admin(username, password, full_name, role, email):
    ...
    try:
        conn.execute(
            "INSERT INTO admins(username, password_hash, full_name, role, email, must_change_pw)"
            " VALUES (?,?,?,?,?,0)",
